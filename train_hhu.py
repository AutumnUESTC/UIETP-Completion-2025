...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------#
    model_path = "/home/wuyou/pspnet-pytorch/model_data/pspnet_resnet50.pth"
    ...
    # ------------------------------------------------------------------#
    VOCdevkit_path = '/home/wuyou/pspnet-pytorch/VOCdevkit'
    # ------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    model = PSPNet(num_classes=num_classes, backbone=backbone, downsample_factor=downsample_factor,\
                   attention=attention, pretrained=pretrained, aux_branch=aux_branch)
    ...
    if Cuda:
        if distributed:
            ...
            if EMA_Train:
                ...
                model_ema = torch.nn.parallel.DistributedDataParallel(model_ema, device_ids=[local_rank],
                                                                      find_unused_parameters=True)
    ...
    with open(os.path.join(VOCdevkit_path, "ImageSets/Segmentation/val.txt"),
              "r") as f:
    ...
    if local_rank == 0:
        show_config(
            ...
            HL_Train=HL_Train, HL_a=HL_a, HL_c=HL_c, EMA_Train=EMA_Train, Trans_branch=trans_branch,
            attention=attention, \
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, EMA_Train, ema, epoch,\
                          ...
                          cls_weights, aux_branch, num_classes, fp16, scaler, save_period, save_dir, distributed,
                          local_rank)
