...
from nets.pspnet_improve import PSPNet
...
if __name__ == "__main__":
    ...
    #   
    #   当model_path = "model_data/pspnet_mobilenetv2.pth"
    #
    #   此处使用的是整个模型的权重，因此是在train.py进行加载的，pretrain不影响此处的权值加载。
    #   如果想要让模型从主干的预训练权值开始训练，则设置model_path = "model_data/pspnet_mobilenetv2.pth"
    #   如果想要让模型从0开始训练，则设置model_path = "model_data/pspnet_mobilenetv2.pth"
    #   
    #   一般来讲，网络从0开始的训练效果会很差，因为权值太过随机，特征提取效果不明显，因此非常、非常、非常不建议大家从0开始训练！
    #   如果一定要从0开始，可以了解imagenet数据集，首先训练分类模型，获得网络的主干部分权值，分类模型的 主干部分 和该模型通用，基于此进行训练。
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "/home/wuyou/pspnet-pytorch/code/model_data/pspnet_mobilenetv2.pth"
    # pspnet_resnet50.pth/pspnet_mobilenetv2.pth
    #---------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    #   训练分为两个阶段，分别是冻结阶段和解冻阶段。设置冻结阶段是为了w满足机器性能不足的同学的训练需求。
    ...
    #   EMA_Train       是否采用EMA
    #   HL_Train        是否采用辅助分支训练
    #   HL_a            权重 一般为0.001、0.01、0.1、1
    #   HL_c            偏置量 一般为0、0.01、0.1、0.5
    #   torch.sigmoid(HL_map) + HL_c / (HL_map.sum())
    #   trans_branch : 0 不使用ViT; 1——并行; 2——串行
    #   attention : 0 不使用; 1 SE; 2 CA; 3 CBAM; 4 GAM; 5 CA;
    #------------------------------------------------------------------#
    Freeze_Train        = False
    EMA_Train           = False
    HL_Train            = False
    ...
    #   weight_decay    权值衰减，可防止过拟合 weight_decay = 1e-4
    ...
    #------------------------------------------------------------------#
    save_dir            = '/home/wuyou/pspnet-pytorch/result/improve/logs-9'
    #------------------------------------------------------------------#
    #   eval_faaaaaaalag       是否在训练时进行评估，评估对象为验证集
    #   eval_period     代表多少个epoch评估一次，不建议频繁的评估a
    ...
    #------------------------------------------------------------------#
    VOCdevkit_path  = '/home/wuyou/pspnet-pytorch/dataset'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    if sync_bn and ngpus_per_node > 1 and distributed:
        ...
        if EMA_Train:
            model_ema = torch.nn.SyncBatchNorm.convert_sync_batchnorm(ema)
    elif sync_bn:
    ...
    if Cuda:
        if distributed:
            ...
            if EMA_Train:
                model_ema = model_ema.cuda(local_rank)
                model_ema = torch.nn.parallel.DistributedDataParallel(model_ema, device_ids=[local_rank], find_unused_parameters=True)
            ...
            if EMA_Train:
                ...
                model_ema = torch.nn.DataParallel(ema)
    ...
    if local_rank == 0:
        show_config(
            num_classes = num_classes, backbone = backbone, model_path = "model_data/pspnet_mobilenetv2.pth"
            ...
            HL_Train = HL_Train, HL_a = HL_a, HL_c = HL_c, EMA_Train = EMA_Train,Trans_branch = trans_branch, attention = attention,\
            save_period = save_period, save_dir = "logs"
        )
        # print(model_train)
        #---------------------------------------------------------#
        #   总训练世代指的是遍历全部数据的总次数
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, EMA_Train, ema, epoch,
