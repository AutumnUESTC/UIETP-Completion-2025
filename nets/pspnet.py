...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone, pretrained=True, aux_branch=True,
        ...
        if backbone == 1:
            ...
            out_channel = 2048
            aux_channel = 1024  # 添加缺失的aux_channel定义
        elif backbone == 2:
        ...
        if self.Att:
            ...
            if self.Att == 4:
                self.Attention = GAM_Attention(out_channel, out_channel)
            if self.Att == 5:
                self.Attention = CoordAtt(out_channel, out_channel)
...
if __name__ == '__main__':
    ...
    pspnet = PSPNet(num_classes=6, downsample_factor=8,
                    backbone=1, pretrained=False, aux_branch=False,
