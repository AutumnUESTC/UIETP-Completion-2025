...
from nets.Attention import SE_Block2, CBAM_block
...
class _PSPModule(nn.Module):
    def __init__(self, in_channels, pool_sizes, norm_layer, trans_branch=None, attention=None):
        ...
        self.bottleneck = nn.Sequential(
            ...
            #           bias=False),
            nn.Conv2d(in_channels + (out_channels * len(pool_sizes)), in_channels + (out_channels * len(pool_sizes)), \
                      kernel_size=3, padding=1, groups=in_channels + (out_channels * len(pool_sizes)), bias=False),
            nn.Conv2d(in_channels + (out_channels * len(pool_sizes)), out_channels, kernel_size=1, bias=False),
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone=1, pretrained=True, aux_branch=True,
        ...
        if self.aux_branch:
            ...
            self.conv = nn.Conv2d(out_channel // 4 + 6, num_classes, kernel_size=1)
        else:
            self.conv = nn.Conv2d(out_channel // 4, num_classes, kernel_size=1)

        self.initialize_weights(self.conv)

        self.Att = attention
        if self.Att:
            self.Attention = SE_Block2(out_channel)
            self.Attention_aux = SE_Block2(aux_channel)
    ...
    def forward(self, x):  # x为input
        ...
        if self.Att:
            x = self.Attention(x)
            x_auxb = self.Attention_aux(x_aux)
        ...
        if self.aux_branch:
            ...
            output_auxb = self.auxiliary_branch(x_auxb)
            # print(output_aux.shape)
            pyramids = [output_auxb]
            pyramids.extend([output])
...
if __name__ == '__main__':
    ...
    pspnet = PSPNet(num_classes=6, downsample_factor=8,
                    backbone=1, pretrained=False, aux_branch=True,
