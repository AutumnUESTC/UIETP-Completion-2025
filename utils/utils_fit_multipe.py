...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, EMA_Train, ema,
                  epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch,
                  cuda, dice_loss, focal_loss, cls_weights, aux_branch, num_classes,
                  fp16, scaler, save_period, save_dir, distributed, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        with torch.no_grad():
            ...
            if cuda:
                if distributed:
                    imgs = imgs.to('cuda:0')
                    pngs = pngs.cuda('cuda:0')
                    labels = labels.cuda('cuda:0')
                    weights = weights.cuda('cuda:0')
                else:
        ...
        if not fp16:
            if aux_branch:
                ...
                if dice_loss:
                    ...
                    loss = (loss + aux_dice * 0.4) * multiple + main_dice
            else:
                #----------------------#
                #   前向传播
                #----------------------#
